...
class Crawler():
    ...
    def syntax_translator(self, command):
        ...
        syntax = re.sub(
            r'(?i)\b(\\\"[A-Za-z0-9_ \t]*\\\"|boardName|weapon|taskName|magazine|name|string|text|subject|skillName|texture|markerName|soundName)\b', '<string>', syntax)
        syntax = re.sub(r'(?i)\b(object|obj|target)\b', '<object>', syntax)
        syntax = re.sub(
            r'(?i)\b(unit|player|person|unitName|killer|killed|member|anObject)\b', '<objUnit>', syntax)
        ...
        syntax = re.sub(
            r'(?i)\b(bool|boolean|true|false|enabled|enable|locked)\b', '<bool>', syntax)
        syntax = re.sub(
            r'(?i)\b(idc|index|objectId|delay|distance|damage|ownerID|points|value|n|id|order|coef|row|column|scale|speed|altitude|color|number|time|zoom)\b', '<scalar>', syntax)
        ...
        syntax = re.sub(
            r'(?i)\b(leaderPos|array|pos|rotation)\b', '<array>', syntax)
    ...
    def _call_bi_wiki(self, command):
        ...
        if 'Description:' in bi_wiki:
            self.fjson[command]['description'] = bi_wiki[bi_wiki.index(
                'Description:') + 1]

        if 'Example 1:' in bi_wiki:
            self.fjson[command]['example'] = bi_wiki[bi_wiki.index(
                'Example 1:') + 1]

        if 'Syntax:' in bi_wiki:
            self.fjson[command]['docSyntax'] = bi_wiki[bi_wiki.index(
                'Syntax:') + 1]
    ...
    def _crawl_game_versions(self, game):
        ...
        versions = json.load(
            open(f'{self.path}/commands/{game}.json'))
    ...
    def _json_output(self, game):
        ...
        cfile = os.path.join(self.path, 'output', f'{game}.min.json')
    ...
    def start(self, game: str, path: str = None):
        ...
        if path:
            self.path = path
        self._crawl_game_versions(game)
...
if (__name__ == '__main__'):
    ...
    parser.add_argument('--all', action='store_true', help='Fetch all games')
